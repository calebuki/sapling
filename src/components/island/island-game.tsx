...
import {
  ...
  DialogTitle,
  DialogDescription,
} from "@/components/island/ui/dialog";
import {
  Tabs,
  TabsList,
  TabsTrigger,
...
import { TooltipProvider } from "@/components/island/ui/tooltip";
import {
  ITEMS,
  RECIPES,
  COLORS,
...
export default function IslandGame({
  ...
  onActivity: (activity: IslandActivity) => void;
  recommended: string;
}) {
  const { states: learningStates, practiceSnapshot } = useLearningModel();
  const successfulRetrievals = learningStates.reduce(
    (n, s) => n + s.successfulRetrievalCount,
    0,
  );
  const evidence = {
    completedScenarioIds: practiceSnapshot.completedScenarioIds,
  ...
  function interact(id: string) {
    if (resources.includes(id as Item))
      // eslint-disable-next-line react-hooks/purity -- This is the world input/arrival event callback, never called during render.
    ...
    else if (id === "school") onActivity("learn");
    else if (id === "elin") onActivity(invitation.id);
    else if (id === "picnic") onActivity("make-weekend-plans");
    else if (id === "ferry") onActivity("centralstation-change");
    return false;
  }
  ...
  return (
    <TooltipProvider delay={120}>
      <section
        ...
        className={`game ${activityOpen ? "activity-open" : ""}`}
      >
        {loaded && !studio && !editing && !placement && (
          <aside className="island-hub" aria-label="Island adventures">
            <div className="island-hub-title">
              <span>🌱</span>
              <div>
                <small>YOUR SAPLING ISLAND</small>
                <b>
                  {state.islandName === "Lilla" ? "Lillaön" : state.islandName}
                </b>
              </div>
              <button
                aria-label="Open journal"
                onClick={() => onActivity("journal")}
              >
                📖
              </button>
            </div>
            <button
              className="island-next"
              onClick={() => onActivity(invitation.id)}
            >
              <span>{invitation.icon}</span>
              <div>
                <small>
                  {practiceSnapshot.completedScenarioIds.includes(invitation.id)
                    ? "A familiar face is waiting"
                    : "An invitation from Elin"}
                </small>
                <b>{invitation.target}</b>
              </div>
              <span>↗</span>
            </button>
            <div className="island-hub-actions">
              <button onClick={() => onActivity("learn")}>
                🌱 Learn a little
              </button>
              <button onClick={() => onActivity("invitations")}>
                ✉ Invitations
              </button>
            </div>
            {availableSupplies(state, evidence).length > 0 && (
              <button
                className="island-supply"
                onClick={() => {
                  const next = claimSupplies(current.current, evidence);
                  ...
                  setToast(
                    "A delivery from Elin! Shells, wood, and stone are in your bag.",
                  );
                }}
              >
                🎁 Your learning earned a supply delivery · collect
              </button>
            )}
          </aside>
        )}
                  ...
                  <button
                    onClick={() => {
                      const url = URL.createObjectURL(
                        new Blob([JSON.stringify(current.current)], {
                          type: "application/json",
                        }),
                      );
                      const a = document.createElement("a");
                      a.href = url;
                      a.download = "sapling-island.json";
                      a.click();
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        if (
                          raw?.version !== 1 ||
                          !raw.bag ||
                          !Number.isInteger(raw.completed)
                        )
                          throw new Error("Not an island save");
                        const restored = restore(text);
                        if (
                          restored.completed !== raw.completed ||
                          restored.coins !== raw.coins ||
                          Object.keys(ITEMS).some(
                            (k) => restored.bag[k as Item] !== raw.bag[k],
                          )
                        )
                        ...
                        const next = {
                          ...restored,
                          learningRewards:
                            current.current.learningRewards ?? [],
                        };
                        ...
                        current.current = next;
                        setState(next);
                        setToast("Your island is home.");
                      } catch {
                        setToast(
                          "Could not import that island file. Your current island is safe.",
                        );
                      }
                    }}
                  />
