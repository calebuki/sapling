...
> = {
  ...
  elin: { icon: "☕", name: "Visit Elin", sv: "Fika med Elin", y: 2.2 },
  picnic: { icon: "🌿", name: "Make plans", sv: "Gör en plan", y: 1.6 },
  ferry: {
    icon: "⛵",
    name: "Plan a little trip",
    sv: "En liten resa",
    y: 1.5,
  },
...
export default function World({
  ...
  useEffect(() => {
    ...
    const growingSapling = new T.Group();
    growingSapling.position.set(-0.5, 0.78, 1.7);
    scene.add(growingSapling);
    mesh(
      new T.CylinderGeometry(0.28, 0.23, 0.35, 10),
      "#cb9978",
      0,
      0.17,
      0,
    ...
    growingSapling.add(canopy);
    box(0.07, 0.7, 0.07, "#927050", 0, 0.35, 0, canopy);
    ball(0.35, "#89b768", 0, 0.85, 0, canopy);
    ball(0.23, "#afcc72", 0.2, 0.65, 0, canopy);
    const invitationTable = new T.Group();
    scene.add(invitationTable);
    box(1.3, 0.12, 0.85, "#f2d198", -2.6, 1.4, 0.1, invitationTable);
    for (const x of [-3.05, -2.15])
      box(0.13, 0.6, 0.13, "#a9835c", x, 1.05, 0.1, invitationTable);
    box(0.7, 0.15, 0.4, "#a8c4a0", -2.6, 1.04, -0.6, invitationTable);
    mesh(
      new T.CylinderGeometry(0.12, 0.1, 0.19, 12),
      "#fff6dc",
      -2.6,
      1.55,
      0.1,
    ...
    box(0.13, 0.9, 0.13, "#ae8965", 2.8, 1.17, 0);
    box(0.85, 0.55, 0.22, "#6f9d87", 2.8, 1.78, 0);
    box(0.54, 0.06, 0.05, "#fff2ca", 2.8, 1.8, 0.13);
    box(1.2, 0.04, 0.9, "#d8aa8d", 3, 0.79, 2);
    ...
    function animate(now: number) {
      ...
      canopy.scale.setScalar(
        0.5 + Math.min(4, live.current.learningGrowth) * 0.28,
      );
