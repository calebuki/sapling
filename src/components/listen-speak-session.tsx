...
export function ListenSpeakSession() {
  ...
  const activeRecognizer = useRef<{ close: () => void } | null>(null);
        ...
        recognizer.recognizeOnceAsync(
          (result) => {
            try {
              ...
              reject(assessmentError);
            }
          },
          (recognitionError) => reject(new Error(recognitionError)),
        );
      }).finally(() => {
        if (activeRecognizer.current === recognizer) {
          recognizer.close();
          audioConfig.close();
          activeRecognizer.current = null;
          activeAudioConfig.current = null;
          ...
          <div>
            ...
            <div className="mt-7 rounded-[22px] border border-forest-900/10 bg-white/55 p-5">
              ...
              >
                ...
                {isRecording ? "Listening—say the sentence" : "Start speaking"}
              </button>
              {isRecording ? (
                <p className="mt-3 flex items-center gap-2 text-xs font-semibold text-clay-400">
